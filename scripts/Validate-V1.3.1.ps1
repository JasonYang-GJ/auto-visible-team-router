...
$required = @(
    ...
    'scripts\Validate-V1.2.ps1','scripts\Validate-V1.3.ps1','scripts\Validate-V1.3.1.ps1',
    'scripts\Run-Acceptance-V1.3.1.ps1','tests\Test-ThreadRegistry.ps1',
...
if ($failures.Count -eq 0) {
    ...
    Assert-Router ($version -eq '1.3.1') 'VERSION is not 1.3.1.'
    ...
    Assert-Router ($wrapper.Contains("'Validate-V1.3.1.ps1'")) 'Validate-V1.ps1 does not target the current validator.'
    ...
    foreach ($requiredText in @('hard fast path','exact-scope','READ_ONLY_STATE_CHANGED','migration-integration.md','Integration Owner')) { Assert-Router ($skill.Contains($requiredText)) "V1.3.1 entry rule is missing: $requiredText" }
    ...
    Assert-Router ($lifecycle.Contains('current V1.3.1 policy')) 'Current lifecycle text still names an obsolete active policy.'
    ...
    Assert-Router ($manager.Contains('自动可视化团队路由器 V1.3.1') -and $manager.Contains("version = '1.3.1'")) 'Managed AGENTS contract is not V1.3.1.'

    $numbered = [regex]::Matches($acceptance,'(?m)^(\d+)\.\s')
    Assert-Router ($numbered.Count -eq 95) "Expected 95 acceptance items, found $($numbered.Count)."
    if ($numbered.Count -eq 95) { for($i=1;$i -le 95;$i++){ Assert-Router ([int]$numbered[$i-1].Groups[1].Value -eq $i) "Acceptance numbering breaks at $i." } }
    ...
    if ($Mode -eq 'Installed') {
        ...
        if (Test-Path -LiteralPath $AgentsPath) {
            ...
            Assert-Router ($agents.Contains('自动可视化团队路由器 V1.3.1')) 'Installed managed block is not V1.3.1.'
        }
    }
}

[pscustomobject]@{
    suite='V1.3.1 static validation'; mode=$Mode; skillRoot=$SkillRoot; version=$(if($version){$version}else{$null})
