...
function Invoke-HistoricalCheck([string]$Name,[string]$Root,[string]$ValidatorName) {
    ...
    if ($resolved.Equals($SkillRoot,[System.StringComparison]::OrdinalIgnoreCase)) {
        Add-Result 'HistoricalSnapshot' $Name 'FAIL' 'Historical validator refused the current V1.3.1 root.'
        return
    }
...
Invoke-PowerShellCheck 'Static' 'Current V1.3.1 validator' (Join-Path $SkillRoot 'scripts\Validate-V1.3.1.ps1') $staticArgs
...
[pscustomobject]@{
    suite='V1.3.1 acceptance truthfulness'; mode=$Mode; skillRoot=$SkillRoot; overall=$overall
