...
function Invoke-HistoricalCheck([string]$Name,[string]$Root,[string]$ValidatorName) {
    ...
    if ($resolved.Equals($SkillRoot,[System.StringComparison]::OrdinalIgnoreCase)) {
        Add-Result 'HistoricalSnapshot' $Name 'FAIL' 'Historical validator refused the current V1.3.3 root.'
        return
    }
...
Invoke-PowerShellCheck 'Static' 'Current V1.3.3 validator' (Join-Path $SkillRoot 'scripts\Validate-V1.3.3.ps1') $staticArgs
...
$behaviorTests = @(
    ...
    'Test-VisibleThreadDeliveryRecovery.ps1','Test-DeliveryReliability.ps1'
)
...
Invoke-HistoricalCheck 'V1.3.2 frozen validator' $HistoricalV132Root 'Validate-V1.3.2.ps1'
...
[pscustomobject]@{
    suite='V1.3.3 acceptance truthfulness'; mode=$Mode; skillRoot=$SkillRoot; overall=$overall
